...
class RemoveShardsCommand extends \CortexPE\Commando\BaseCommand
{
    ...
    {
        $this->setPermission("command.remove_shards");
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if ($session->getShards() < $amount) {
            $sender->sendMessage(TextFormat::colorize("&r&l&c(!) &r&cThe player '&f" . $player->getName() . "&c' does not have enough shards to remove."));
            return;
        }

        $session->subtractShards($amount);
